...
if(isset($_POST['submit']))
{
    ...
    $prefix = 'DN';
	$randomString = '';
	// $length = 2; // Desired length of the random string

	// for ($i = 0; $i < $length; $i++) {
	// 	$randomString .= $alphabets[array_rand($alphabets)]; // Randomly select an alphabet and concatenate it to the string
	// }
    $randomString = substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 2); // Generates a random 6-character alphanumeric string
    $timestamp = time(); // Current timestamp
    $donorId = $prefix . $randomString . substr($timestamp, -7);
	...
	if($query -> rowCount() == 0)
	{
	$sql="INSERT INTO  tblblooddonars(donorId,FullName,MobileNumber,EmailId,Age,Gender,BloodGroup,Address,status,Password) VALUES(:donorId,:fullname,:mobile,:email,:age,:gender,:blodgroup,:address,:status,:password)";
